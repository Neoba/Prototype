 ...
 * @author atul
 */
public class IosGetDigestMessage implements Message {
    ...
    byte[] arraydiff;
    int arraydiffl;
    HashMap<UUID,String> fulldoc; 
    public IosGetDigestMessage(UUID sessid) throws JSONException, IOException, VcdiffDecodeException {
		
    	fulldoc=new HashMap<UUID,String>();
        this.sessid = sessid;
        ...
        for (int i = 0; i < docs.length(); i++) {
           ...
           // bufsize += ((JSONArray) doc.get("diff")).length();
            ...
            arraydiffl=((JSONArray) doc.get("diff")).length();
            arraydiff=new byte[arraydiffl];
			
           // bufsize += ((String) doc.get("dict")).length();
            source=(String) doc.get("dict");
            for (int k = 0; k < ((JSONArray) doc.get("diff")).length(); k++) {
                arraydiff[k]=((Integer) ((JSONArray) doc.get("diff")).get(k)).byteValue();
            }
			logger.info("Constructor");
            target=new VcdiffDecoder(source,arraydiff).decode();
            fulldoc.put(id, target);
            bufsize+=target.length();
            //bufsize += ((String) doc.get("title")).length();
            bufsize += (16 + 4 + 4 + 4 + 4-4-4);
            bufsize += 1;
            bufsize+=1;
            doccount += 1;
        }
    ...
    public ByteBuf result() {
        ...
        reply.writeByte(Constants.IOS_GET_DIGEST);
        ...
        try {
            for (int i = 0; i < docs.length(); i++) {
                ...
                reply.writeInt(fulldoc.get(id).length());
                for (byte b : ((String) fulldoc.get(id)).getBytes()) {
                    reply.writeByte(b);
                }
