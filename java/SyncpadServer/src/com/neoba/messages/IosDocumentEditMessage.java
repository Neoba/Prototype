...
public class IosDocumentEditMessage implements Message {
	Boolean haspermission = false, documents_are_synced = false;
    Boolean creatoredited=false;
    private boolean push_success = true;//Ios Push to be implemented 
    Logger logger=Logger.getLogger(IosDocumentEditMessage.class);
    String target;
    String dict;
    byte[] diff;
    int age;
    public IosDocumentEditMessage(UUID doc, byte[] text, int version, UUID sessid) throws JSONException, IOException, VcdiffDecodeException, VcdiffEncodeException, Exception
    {
        ...
        diff=new VcdiffEncoder(dict,target).encode();
        ...
        if (haspermission) {
            ConsoleUtils.printhex("diff_calculated",diff, diff.length);
            age += 1;
            if (age % 5 == 0) {
            ...
            logger.info(sessid+" 's copy: " + target);
    ...
    public ByteBuf result() {
        ...
        reply.writeByte(Constants.IOS_DOCUMENT_EDIT);
