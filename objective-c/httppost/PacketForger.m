...
+(UInt16)editDocUsercookie:(NSUUID *)cookie docid:(NSUUID *)docid text:(NSString *)text age:(int)age
{
    const Byte code=0xD;
    UInt16 rstatus;
    uuid_t cookieflat,docidflat;
    ...
    textl=OSSwapBigToHostInt32(textl);
    ctext=malloc(textl);
    
    [text getCString:ctext maxLength:200 encoding:NSUTF8StringEncoding];
    NSMutableData *body=[[NSMutableData alloc] init],*result=nil;
    if (age==-1) {
        age=0;
    }
    else
    age=OSSwapBigToHostInt32(age+1);
    //NSLog(@"%d",textl);
    [body appendBytes:&ver length:1];
    [body appendBytes:&code length:1];
    [body appendBytes:&textl length:4];
    [body appendBytes:cookieflat length:16];
    [body appendBytes:docidflat length:16];
    textl=OSSwapBigToHostInt32(textl);
    [body appendBytes:ctext length:textl];
    [body appendBytes:&age length:4];
...
+(void)getDigestwith:(NSUUID *)cookie followers:(NSMutableDictionary *)dfollowers followings:(NSMutableDictionary *) dfollowings docs:(NSMutableArray *) docs
{
    ...
    if ([dgst length]>6) {    //No success code for gd, so relying on length
        ...
        for (int i=0; i<doccount; ++i) {
            ...
            range=NSMakeRange(base+24+sdiff+sdict, 4);
...
+(void) iosgetDigestwith:(NSUUID *)cookie followers:(NSMutableDictionary *)dfollowers followings:(NSMutableDictionary *) dfollowings docs:(NSMutableArray *) docs
{
    const Byte code=0xE;
    int status=0xFFFF;
    status=OSSwapBigToHostInt32(status);
    ...
    dgst=[Sendpost sendreq:body];
     if ([dgst length]>6) {    //No success code for gd, so relying on length
        ...
        for (int i=0; i<doccount; ++i) {
            ...
            range=NSMakeRange(base, 16);
            [dgst getBytes:docidflat range:range];
            doc.did=[[NSUUID alloc] initWithUUIDBytes:docidflat];
            range=NSMakeRange(base+16, 4);
            [dgst getBytes:&stext range:range];
            stext=OSSwapBigToHostInt32(stext);
            char *ctext=malloc(stext+1);
            range=NSMakeRange(base+20, stext);
            [dgst getBytes:ctext range:range];
            ctext[stext]='\0';
            doc.text=[[NSString alloc] initWithCString:ctext encoding:NSUTF8StringEncoding];
            base = base + 16+4+stext;
            range=NSMakeRange(base,4);
            [dgst getBytes:&age range:range];
            base+=4;
            range=NSMakeRange(base,1);
            [dgst getBytes:&per range:range];
            doc.permission=per;
            ++base;
            range=NSMakeRange(base,1);
            [dgst getBytes:&ow range:range];
            doc.owns=(ow==0x1);
            base+=1;
            
            NSLog(@"%d--%d",base,age);
            age=OSSwapBigToHostInt32(age);
            doc.age=age;
            
            
            
            if (![docs containsObject:doc]) {
                [docs addObject:doc];
            }
