...
public class DBManager {
	...
	public boolean isDocUnSynced(String docid) {
		String sql = "SELECT * from docs where id=? and (synced<>0 or syncede<>0)";
		Cursor c = db.rawQuery(sql, new String[] { docid });
		if (c.getCount() <= 0) {
			return true;
		}
	...
	public void insertPermission(String docid, String username, Long userid,
			int permission) {
	...
	public String getFollowerUsername(Long userid) {
		...
		Cursor cursor = db
				.rawQuery(sql, new String[] { Long.toString(userid) });
		cursor.moveToFirst();
	...
	public String getDoc(String id) throws IOException, VcdiffDecodeException {
		...
		Log.d("DICTGETDOC", dict + Arrays.toString(diff) + ":"
				+ new VcdiffDecoder(dict, diff).decode());
	...
	public void clearPermissions(String docid) {
		SQLiteStatement updateStmt = db
				.compileStatement("DELETE from permissions where docid=?");
		updateStmt.clearBindings();
		updateStmt.bindString(1, docid);
		updateStmt.executeUpdateDelete();
	}
	...
	public void clearPermissions(String docid, String userid) {
		SQLiteStatement updateStmt = db
				.compileStatement("DELETE from permissions where docid=? and userid=?");
		updateStmt.clearBindings();
		updateStmt.bindString(1, docid);
	...
	public void insertDocFromDigest(document doc) throws IOException, VcdiffDecodeException {
			...
			insertStmt.bindString(3, doc.dict);
			insertStmt.bindLong(4, doc.age);
			insertStmt.bindString(5, new VcdiffDecoder(doc.dict, doc.diff).decode());
			insertStmt.bindLong(6, doc.permission);
			insertStmt.bindLong(7, new Date().getTime());
			insertStmt.bindLong(8, doc.owns ? 1 : 0);
